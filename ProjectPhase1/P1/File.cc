...
Record* Page :: next() {
	//cout<<myRecs->RightLength()<<"len"<<endl;
	if(myRecs->RightLength() <= pointer)return NULL;
	Record *record = myRecs->Current(pointer);
	pointer++;
	return record;
}
...
int File :: Open (int fileLen, char *fName) {
	...
	struct stat st;
                ...
                curLength = st.st_size/PAGE_SIZE;
////////////////	cout << "Opening file " << fName << " with "<< curLength << " pages. where myFileDes is\n"<<myFilDes<<endl;
