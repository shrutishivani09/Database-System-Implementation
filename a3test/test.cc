...
int pipesz = 100000000; // buffer sz allowed for each pipe
int buffsz = 100000000; // pages of memory allowed for operations
...
void q2 () {
	...
	// while(_out.Remove(new Record()) == 1)cout<<"koushik"<<endl;
...
void q3 () {
	...
	Record record;
	// while(_s.Remove(&record))cout<<cnt++<<endl;
	// cout<<"Higaya";
	// cout<<"hellllooooo"<<endl;
	T.Run (_s, _out, func);
...
void q4 () {
	...
	// while(_ps.Remove(new Record()))cnt++;
	// cout<<cnt;
	J.Run (_s, _ps, _s_ps, cnf_p_ps, lit_p_ps);

	Record record;
	// cout<<_s_ps.Remove(&record);
	// while(_s_ps.Remove(&record))record.Print(&join_sch);
...
void q5 () {
	...
	// int cnt = 0;
	// while(___ps.Remove(new Record()))cnt++;
	// cout<<cnt<<endl;
